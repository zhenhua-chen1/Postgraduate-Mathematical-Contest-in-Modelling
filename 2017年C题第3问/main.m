...
aircrafts_num=zeros(length(flights(:,5)),1);
...
Schedules_num=flights(1:length(flights(:,5)),6);
Reduced_numbers=zeros(length(flights(:,5)),1);
for f=1:length(flights(:,5))
    aircrafts{f}=No_Aircrafts{flights(f,5),1};
    aircrafts_type{f}=No_Aircrafts{flights(f,5),2};
    delay_time(f) = time(flights(f,5),f);
    aircrafts_num(f)=Aircrafts(flights(f,5),4);
    if Schedules_num(f)>aircrafts_num(f)
        Reduced_numbers(f)=Schedules_num(f)-aircrafts_num(f);
    end
end



T = table(Schedules,Schedules_type,aircrafts,aircrafts_type,delay_time,Schedules_num,aircrafts_num,Reduced_numbers);
writetable(T,'第三问新航班表.xlsx');
