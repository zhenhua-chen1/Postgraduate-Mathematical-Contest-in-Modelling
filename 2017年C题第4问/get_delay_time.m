function [f,Aij_time,cancel,Passanage_time,time2]=get_delay_time(Aij)
    load data
    Aij=~Aij;
    [n_a,n_f]=size(Aij);
   ...
   for i=1:I
       for j=1:J
           if Aij(i,j)==1
               ...
               if Aij_time(i,j)~=0
                   Passanage_time(i,j)=flights(j,6)*0.5*3600;
                   f=f+Passanage_time(i,j);
               end  
               if flights(j,6)>Aircrafts(i,4)
                   f=f+2*3600*(flights(j,6)-Aircrafts(i,4));
                   Aij_time(i,j)=Aij_time(i,j)+2*3600*(flights(j,6)-Aircrafts(i,4));
               end
           end
       end
   end
