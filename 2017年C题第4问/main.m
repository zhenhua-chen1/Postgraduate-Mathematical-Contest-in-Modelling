...
[fitness,time,c,passange,time2]=get_delay_time(Aij1);
...
replace_time=zeros(length(flights(:,5)),1);
Passengers_time=zeros(length(flights(:,5)),1);
...
for f=1:length(flights(:,5))
    ...
    Passengers_time(f)=passange(flights(f,5),f);
    replace_time(f) =time2(flights(f,5),f); 
end



T = table(Schedules,Schedules_type,aircrafts,aircrafts_type,delay_time,replace_time,Passengers_time);
writetable(T,'第四问新航班表.xlsx');
