function output_flight(path_all1,path_all2,I2)
    [a1,b]=size(path_all1);
    [a2,b]=size(path_all2);
    rawA = readcell('机组排班Data B-Flight.xlsx');
    ...
    for p=1:a2
        ...
        for  F=2:length(temp_raw)
          ...
          for i=1:length(temp_path)
              path=temp_path(i);
              twt=[twt;[Captain,i,rawA(path,2:8),'C']];
              twt=[twt;[FirstOfficer,i,rawA(path,2:8),'F']];
          end
    end 
    for i=I2
       twt2=[twt2;rawA(i,2:9)];
    end
     writecell(twt,'UncoveredFlights.xlsx')
     writecell(twt2,'CrewRosters.xlsx')
end